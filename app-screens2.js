...
function buildEntHTML(di){
  ...
  const semSel=`<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;background:var(--az3);border-radius:10px;margin-bottom:10px">
    <button onclick="if(ST.semVer>1){ST.semVer--;buildDIAS(ST.semVer);restoreEjStatesFromHistEnt(ST.semVer);save();document.getElementById('ct').innerHTML=renderEntreno();}" style="background:none;border:none;font-size:20px;cursor:pointer;color:var(--az);padding:4px 8px" ${(ST.semVer||semActual)<=1?'disabled':''}>‹</button>
    ...
    <button onclick="if((ST.semVer||1)<${semTotal}){ST.semVer++;buildDIAS(ST.semVer);restoreEjStatesFromHistEnt(ST.semVer);save();document.getElementById('ct').innerHTML=renderEntreno();}" style="background:none;border:none;font-size:20px;cursor:pointer;color:var(--az);padding:4px 8px" ${(ST.semVer||semActual)>=semTotal?'disabled':''}>›</button>
  </div>
