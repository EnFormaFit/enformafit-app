...
function buildEntHTML(di){
  ...
  const semSel=`<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;background:var(--az3);border-radius:10px;margin-bottom:10px">
    <button onclick="if(ST.semVer>1){ST.semVer--;buildDIAS(ST.semVer);restoreEjStatesFromHistEnt(ST.semVer);document.getElementById('ct').innerHTML=renderEntreno();}" style="background:none;border:none;font-size:20px;cursor:pointer;color:var(--az);padding:4px 8px" ${(ST.semVer||semActual)<=1?'disabled':''}>‹</button>
    ...
    <button onclick="if((ST.semVer||1)<${semTotal}){ST.semVer++;buildDIAS(ST.semVer);restoreEjStatesFromHistEnt(ST.semVer);document.getElementById('ct').innerHTML=renderEntreno();}" style="background:none;border:none;font-size:20px;cursor:pointer;color:var(--az);padding:4px 8px" ${(ST.semVer||semActual)>=semTotal?'disabled':''}>›</button>
  </div>
...
function renderEj(ej,ei,di,grabIdxs){
  ...
  st.series.forEach((s,si)=>{
    ...
    sers+=`<div class="serrow">
      ...
      <input class="sinp" type="number" inputmode="decimal" placeholder="kg" min="0" value="${s.kg}" oninput="if(parseFloat(this.value)<0)this.value=0;ST.ejStates['${key}'].series[${si}].kg=this.value;autoGuardarSerie(${di},${ei},${si})" onchange="ST.ejStates['${key}'].series[${si}].kg=this.value;autoGuardarSerie(${di},${ei},${si})">
      <input class="sinp" type="number" inputmode="numeric" placeholder="${ej.reps}" min="0" value="${s.repsH}" oninput="if(parseFloat(this.value)<0)this.value=0;ST.ejStates['${key}'].series[${si}].repsH=this.value;autoGuardarSerie(${di},${ei},${si})" onchange="ST.ejStates['${key}'].series[${si}].repsH=this.value;autoGuardarSerie(${di},${ei},${si})">
      <input class="sinp" type="number" inputmode="numeric" placeholder="RIR" max="5" value="${s.rir||''}" oninput="ST.ejStates['${key}'].series[${si}].rir=this.value;autoGuardarSerie(${di},${ei},${si})" onchange="ST.ejStates['${key}'].series[${si}].rir=this.value;autoGuardarSerie(${di},${ei},${si})" style="color:var(--nr)">
...
function toggleSer(key,si,di){
  ...
  // Update histEnt immediately so week changes preserve this
  const _tParts=key.split('_');const _tDi=parseInt(_tParts[0]);const _tEi=parseInt(_tParts[1]);
  const _tDia=DIAS[_tDi];const _tEj=_tDia&&_tDia.ejercicios&&_tDia.ejercicios[_tEi];
  if(_tEj) updateHistEnt(_tEj.nom||_tEj.nombre,_tDi,si,ST.semVer||ST.u.semana||1,s.kg,s.repsH,s.rir,s.done);
  save();
  if(s.done){
  ...
  <div class="cb">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
      ...
      <div>
        ...
        <input type="number" inputmode="decimal" id="pesoVal" placeholder="0.0" min="0" step="0.1" style="width:100%;border:1.5px solid var(--bor);border-radius:9px;padding:10px 8px;font-size:15px;font-weight:700;outline:none;font-family:inherit;color:var(--az)">
      </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:7px;margin-bottom:14px">
      ...
      <div class="maci"><div class="macv" style="color:var(--az2);font-size:17px">${ST.objPeso}kg</div><div class="macl">Objetivo</div></div>
          <div style="background:var(--bg);border-radius:8px;padding:12px;text-align:center"><div style="font-size:20px;font-weight:800;color:var(--nr)">${ST.p.objSemKg?ST.p.objSemKg+'kg/sem':'—'}</div><div style="font-size:10px;color:var(--t3)">Ritmo objetivo</div></div>
    ...
    <div class="card" style="margin-bottom:12px">
      ...
      <div class="cb">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
          ...
          <div style="background:var(--bg);border-radius:8px;padding:12px;text-align:center">
            <div style="font-size:24px;font-weight:800;color:var(--nr)">${ST.p.objSemKg?ST.p.objSemKg+'kg/sem':'—'}</div>
